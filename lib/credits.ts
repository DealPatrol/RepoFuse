...
export async function renewMonthlyCredits(
  ...
  const transaction = await sql`
    ...
    updated AS (
      UPDATE user_credits
      SET
        current_balance = user_credits.current_balance + current.top_up_amount,
        total_granted = total_granted + current.top_up_amount,
        last_renewal_date = CURRENT_TIMESTAMP
      FROM current
      WHERE user_id = ${userId}
      RETURNING user_credits.current_balance, current.top_up_amount
    )
    INSERT INTO credit_transactions (
      user_id, amount, transaction_type, reason, metadata, balance_after
    )
    SELECT ${userId}, top_up_amount, 'renewal', ${reason}, ${JSON.stringify(metadata)}::jsonb, current_balance
    FROM updated
