...
export async function renewMonthlyCredits(
  ...
  const transaction = await sql`
    ...
    updated AS (
      ...
      SET
        current_balance = GREATEST(uc.current_balance, ${monthlyAllowance}),
        total_granted = uc.total_granted + GREATEST(${monthlyAllowance} - uc.current_balance, 0),
        last_renewal_date = CURRENT_TIMESTAMP
      FROM current_credits
      WHERE uc.user_id = ${userId}
      RETURNING
        uc.current_balance AS balance_after,
        GREATEST(${monthlyAllowance} - current_credits.current_balance, 0) AS top_up_amount
    )
    INSERT INTO credit_transactions (
      user_id, amount, transaction_type, reason, metadata, balance_after
    )
    SELECT ${userId}, top_up_amount, 'renewal', ${reason}, ${JSON.stringify(metadata)}::jsonb, balance_after
    FROM updated
    RETURNING *
  `
...
export async function hasCreditTransactionWithMetadata(
  userId: string,
  ...
  const result = await sql`
    ...
    WHERE user_id = ${userId}
      ...
      AND metadata @> ${JSON.stringify(metadata)}::jsonb
    LIMIT 1
  `
