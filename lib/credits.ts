...
export async function renewMonthlyCredits(
  ...
  const transaction = await sql`
    ...
    updated AS (
      UPDATE user_credits
      SET
        current_balance = ${monthlyAllowance},
        total_granted = total_granted + (${monthlyAllowance} - current_row.current_balance),
        last_renewal_date = CURRENT_TIMESTAMP
      FROM current_row
      WHERE user_credits.user_id = ${userId}
        AND current_row.current_balance < ${monthlyAllowance}
      RETURNING user_credits.current_balance, (${monthlyAllowance} - current_row.current_balance) AS top_up_amount
    )
    INSERT INTO credit_transactions (
      user_id, amount, transaction_type, reason, metadata, balance_after
    )
