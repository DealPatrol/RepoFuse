...
export async function renewMonthlyCredits(
  ...
  const transaction = await sql`
    WITH current_row AS (
      SELECT current_balance, GREATEST(${monthlyAllowance} - current_balance, 0) AS top_up
      FROM user_credits
      WHERE user_id = ${userId}
      FOR UPDATE
    ),
    updated AS (
      ...
      SET
        current_balance = uc.current_balance + current_row.top_up,
        total_granted = total_granted + current_row.top_up,
        last_renewal_date = CURRENT_TIMESTAMP
      FROM current_row
      WHERE uc.user_id = ${userId}
      RETURNING uc.current_balance, current_row.top_up
    )
    INSERT INTO credit_transactions (
      user_id, amount, transaction_type, reason, metadata, balance_after
    )
    SELECT ${userId}, top_up, 'renewal', ${reason}, ${JSON.stringify(metadata)}::jsonb, current_balance
    FROM updated
    WHERE top_up > 0
    RETURNING *
  `
