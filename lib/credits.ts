...
export async function deductCredits(
  ...
  const updatedBalance = (updated[0] as CreditBalanceRow | undefined)?.current_balance
  ...
  const transaction = await sql`
    ...
    VALUES (
      ${userId}, ${-amount}, ${type}, ${`${type} deduction`}, ${JSON.stringify(metadata)}::jsonb, ${updatedBalance}
    )
    RETURNING *
  `
...
export async function refundCredits(
  ...
  const newBalance = (updated[0] as CreditBalanceRow).current_balance
