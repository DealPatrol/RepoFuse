    ...
    updated AS (
      ...
      SET
        current_balance = CASE
          WHEN current_row.current_balance < ${monthlyAllowance} THEN ${monthlyAllowance}
          ELSE uc.current_balance
        END,
        total_granted = total_granted + GREATEST(${monthlyAllowance} - current_row.current_balance, 0),
        last_renewal_date = CURRENT_TIMESTAMP
      FROM current_row
      WHERE uc.user_id = ${userId}
      RETURNING
        uc.current_balance,
        GREATEST(${monthlyAllowance} - current_row.current_balance, 0) AS top_up_amount
    )
    INSERT INTO credit_transactions (
      user_id, amount, transaction_type, reason, metadata, balance_after
    )
