...
export async function reserveAnalysisUsage(githubId: number, monthlyLimit: number): Promise<boolean> {
  const sql = getDb()
  const result = await sql`
    UPDATE subscriptions
    SET analyses_used_this_month = analyses_used_this_month + 1, updated_at = CURRENT_TIMESTAMP
...
export async function releaseAnalysisUsage(githubId: number): Promise<void> {
  const sql = getDb()
  await sql`
    UPDATE subscriptions
...
export async function deleteBlueprintsByAnalysisExcept(
  ...
  if (keepIds.length === 0) {
    throw new Error('Cannot replace blueprints without replacement rows')
  }
...
export async function deleteBlueprintsByIds(
  analysisId: string,
  userId: string,
  blueprintIds: string[],
): Promise<void> {
...
export async function deleteMilestone(projectId: string, id: string, userId: string): Promise<boolean> {
  const sql = getDb()
  const result = await sql`
