...
export async function getMilestonesByProject(projectId: string, userId?: string): Promise<ProjectMilestone[]> {
  const sql = getDb()
  const rows = userId
    ? await sql`
        ...
        ORDER BY m.phase, m.sort_order, m.created_at
      `
    : await sql`
...
export async function deleteMilestone(projectId: string, userId: string, id: string): Promise<boolean> {
  const sql = getDb()
  const result = await sql`
