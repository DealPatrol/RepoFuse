...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const creditResult = await deductCredits(user.id, CREDITS.ANALYSIS_COST, 'analysis', {
      analysisId,
      ...
      return NextResponse.json(
        {
          ...
          message: creditResult.error || 'Upgrade to Pro to get unlimited analyses with 3,000 monthly credits.',
        },
        { status: 402 }
      )
    }
...
Return as JSON array of app suggestions. Focus on practical, buildable applications.`
    ...
    if (chargedUserId) {
      await refundCredits(chargedUserId, CREDITS.ANALYSIS_COST, 'Legacy analysis failed', {
        idempotency_key: `legacy-analysis-refund:${chargedUserId}:${chargedAnalysisId ?? 'none'}:${Date.now()}`,
