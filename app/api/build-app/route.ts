    ...
    async start(controller) {
      ...
      try {
        ...
        if (platform !== 'github' && platform !== 'gitlab') {
          send({ step: 'error', message: 'Platform must be github or gitlab.' })
          return
        }
        ...
        const creditResult = await deductCredits(user.id, CREDITS.BUILD_APP_COST, 'build_app', refundMetadata)
        ...
        refundMetadata = { ...refundMetadata, repoUrl }
        ...
        if (chargedUserId && charged && !completed) {
          await refundCredits(chargedUserId, CREDITS.BUILD_APP_COST, 'Build This App failed', {
            ...refundMetadata,
            idempotency_key: `build-app-refund:${chargedUserId}:${Date.now()}`,
          }).catch((refundError) => console.error('[build-app] Failed to refund credits:', refundError))
        }
        controller.enqueue(
          encoder.encode(
