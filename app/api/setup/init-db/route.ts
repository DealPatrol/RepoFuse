...
async function run() {
  try {
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_completed_gaps_user_id ON completed_gaps(user_id)`
    await sql`CREATE INDEX IF NOT EXISTS idx_templates_user_id ON templates(user_id)`
    await sql`CREATE INDEX IF NOT EXISTS idx_blueprint_views_user_id ON blueprint_views(user_id)`
