...
Respond ONLY with a valid JSON object (no markdown fences) matching this exact shape:
{
  ...
  "suggestions": [
    ...
    try {
      ...
      await refundCredits(user.id, CREDITS.PATTERN_ANALYZER_COST, 'Pattern Analyzer failed', { analysisId }).catch((refundError) => {
    ...
    if (charged && chargedUserId && chargedAnalysisId) {
      await refundCredits(chargedUserId, CREDITS.PATTERN_ANALYZER_COST, 'Pattern Analyzer failed', { analysisId: chargedAnalysisId }).catch((refundError) => {
        console.error('[pattern-analyzer] Failed to refund credits:', refundError)
      })
    }
