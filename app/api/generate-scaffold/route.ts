...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    creditMetadata = { appName, technologies }
...
Example structure:
{
  ...
  "files": {
    ...
    if (chargedScaffold && chargedUserId) {
      try {
        await refundCredits(chargedUserId, CREDITS.SCAFFOLD_COST, 'Scaffold generation failed', creditMetadata)
      } catch (refundError) {
        console.error('[scaffold] Failed to refund scaffold credits:', refundError)
      }
    }
