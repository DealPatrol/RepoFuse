...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const creditResult = await deductCredits(user.id, CREDITS.SCAFFOLD_COST, 'scaffold', chargeMetadata)
    if (!creditResult.success) {
      return NextResponse.json(
        {
          error: creditResult.error || 'Insufficient credits',
          required: CREDITS.SCAFFOLD_COST,
          message: 'Upgrade to Pro to get scaffold generation with 3,000 monthly credits.',
        },
        { status: 402 },
      )
    }
...
Example structure:
{
  ...
  "files": {
    ...
    if (chargedUserId) {
      await refundCredits(chargedUserId, CREDITS.SCAFFOLD_COST, 'Scaffold generation failed', chargeMetadata).catch(
        (refundError) => console.error('[scaffold] Failed to refund credits:', refundError),
      )
    }
