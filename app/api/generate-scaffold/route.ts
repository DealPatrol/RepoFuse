...
export async function POST(request: NextRequest) {
  ...
  let appNameForRefund: string | null = null
  ...
  "files": {
    ...
    if (chargedUserId) {
      await refundCredits(chargedUserId, CREDITS.SCAFFOLD_COST, 'Scaffold generation failed', {
        appName: appNameForRefund,
      }).catch((refundError) => console.error('[scaffold] Failed to refund credits:', refundError))
    }
    return NextResponse.json(
